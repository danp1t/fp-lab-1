defmodule ProblemRecTail11 do
  @moduledoc """
  Модуль для решения задачи №11.
  """

  def rec_tail_solution(grid) do
    rows = length(grid)
    cols = length(hd(grid))
    function(grid, 0, 0, 0, rows, cols)
  end

  defp function(_grid, rows, _j, max, rows, _cols), do: max

  defp function(grid, i, cols, max, rows, cols) do
    function(grid, i + 1, 0, max, rows, cols)
  end

  defp function(grid, i, j, max, rows, cols) do
    new_max =
      max
    ...
    function(grid, i, j + 1, new_max, rows, cols)
  end
